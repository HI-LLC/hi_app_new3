$PBExportHeader$w_lesson_comp_object.srw
forward
global type w_lesson_comp_object from w_lesson
end type
end forward
...
global type w_lesson_comp_object from w_lesson
string tag = "1502000"
string title = "Lesson - Comparison (object)"
end type
global w_lesson_comp_object w_lesson_comp_object
...
elseif ii_current_try >= ii_tries then
	if ii_prompt_ind = 2 then // Error Correction
		...
		if FileExists(is_prompt_instruction) then
			...
			MessageBox("Error", "Sound file - " + is_prompt_instruction + " does not exist!")
		end if
		for ll_i = 1 to 1000 
...
if ii_current_list_offset + ii_degree - 1  > ii_total_items then
...
on w_lesson_comp_object.create
call super::create
end on

on w_lesson_comp_object.destroy
call super::destroy
end on
...
type dw_reward from w_lesson`dw_reward within w_lesson_comp_object
end type

type cb_close from w_lesson`cb_close within w_lesson_comp_object
string tag = "1502040"
end type

type cb_start from w_lesson`cb_start within w_lesson_comp_object
string tag = "1502030"
end type

type dw_2 from w_lesson`dw_2 within w_lesson_comp_object
string tag = "1502020"
string title = ""
end type
...
type p_1 from w_lesson`p_1 within w_lesson_comp_object
end type

type st_1 from w_lesson`st_1 within w_lesson_comp_object
end type

type dw_1 from w_lesson`dw_1 within w_lesson_comp_object
string tag = "1502000"
end type

event dw_1::itemchanged;call super::itemchanged;if dwo.name = 'student' then
	wf_lesson_filter('method_id >= 17 and method_id <= 19')
end if
end event

type dw_prompt from w_lesson`dw_prompt within w_lesson_comp_object
integer width = 2491
end type
