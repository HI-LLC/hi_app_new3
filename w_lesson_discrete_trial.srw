$PBExportHeader$w_lesson_discrete_trial.srw
forward
global type w_lesson_discrete_trial from w_lesson
end type
end forward
...
global type w_lesson_discrete_trial from w_lesson
string tag = "1501000"
string title = "Lesson - Object Identification"
windowstate windowstate = normal!
end type
global w_lesson_discrete_trial w_lesson_discrete_trial
...
public subroutine wf_question_announcer ();long ll_time_waited = 0, ll_row
boolean lb_ready =true
...
if ii_degree = 0 then ii_degree = 2
...
public subroutine wf_response ();integer li_row, li_correct_row, il_row, li_i
...
//		inv_sound_play.play_st_sound(is_wave_list[ii_current_question_id])
//	else
////		for li_i = 1 to ii_tries // SPEAKING TRAINING
////			wf_play_video(is_wave_list[ii_current_question_id])
////		next
////		Messagebox("Repeat", is_text_list[ii_current_question_id])	
////		wf_get_new_item()
////		return
//		wf_play_video(is_wave_list[ii_current_question_id])
//	end if
...
public subroutine wf_get_new_item ();integer il_row, li_correct_row, li_delayed_time = 0
...
//			FileDelete(ls_local_filename)
		...
		lw_tmp.visible = true
	elseif pos(ls_local_filename, ".avi") > 0 or pos(ls_local_filename, ".mpg") > 0 or pos(ls_local_filename, ".mov") > 0 then
...
for il_row = 1 to ii_degree
	...
	lw_tmp.pb_1.BringToTop = true
next
//MessageBox("wf_get_new_item", "A")
wf_question_announcer()
//MessageBox("wf_get_new_item", "B")
...
on w_lesson_discrete_trial.create
call super::create
end on

on w_lesson_discrete_trial.destroy
call super::destroy
end on
...
type dw_reward from w_lesson`dw_reward within w_lesson_discrete_trial
end type

type cb_close from w_lesson`cb_close within w_lesson_discrete_trial
string tag = "1501030"
end type

type cb_start from w_lesson`cb_start within w_lesson_discrete_trial
string tag = "1501030"
end type

type dw_2 from w_lesson`dw_2 within w_lesson_discrete_trial
string tag = "1501020"
end type
...
type p_1 from w_lesson`p_1 within w_lesson_discrete_trial
end type

type st_1 from w_lesson`st_1 within w_lesson_discrete_trial
end type

type dw_1 from w_lesson`dw_1 within w_lesson_discrete_trial
string tag = "1501010"
end type

event dw_1::itemchanged;call super::itemchanged;//MessageBox("itemchanged", "itemchanged")
if dwo.name = 'student' then
	wf_lesson_filter('method_id = 2')
end if
end event

type dw_prompt from w_lesson`dw_prompt within w_lesson_discrete_trial
integer width = 2519
end type
